package com.bookie.scrap.watcha.request.user.userwishbook;

import com.bookie.scrap.common.domain.PageInfo;
import lombok.extern.slf4j.Slf4j;
import org.apache.hc.core5.net.URIBuilder;

import java.net.URISyntaxException;

@Slf4j
public class WatchaUserWishBookParam extends PageInfo {

    private String sortDirection;
    private String sortOption;

    public WatchaUserWishBookParam(int page, int size) {
        super(page, size);
    }

    public void setSortDirection() {
        this.sortDirection = "desc";
    }

    public void setSortOption() {
        this.sortOption = "acted_at";
    }

    @Override
    public String buildUrlWithParamInfo(String baseUrl) {
        super.validNotEmpty(baseUrl, "baseUrl");

        try {
            URIBuilder uriBuilder = new URIBuilder(baseUrl)
                    .addParameter("page", String.valueOf(super.page))
                    .addParameter("size", String.valueOf(super.size));

            if (this.sortOption != null) {
                uriBuilder.addParameter("sort_option", this.sortOption);
            }

            if (this.sortDirection != null) {
                uriBuilder.addParameter("sort_direction", this.sortDirection);
            }

            return uriBuilder.build().toString();

        } catch (URISyntaxException e) {
            log.error("Failed to build URL for baseUrl {}", baseUrl);
            throw new IllegalStateException(e);
        }
    }

}
